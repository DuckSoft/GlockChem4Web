import { AdvNum } from "./advnum";

/**
 * 原子量数据库异常清单
 */
export enum RMDatabase_Exceptions {
    /**所查询的原子在原子量数据库中无法找到。*/
 ...
 * @version 0.1
 */
export class RMDatabase {
    rmData: Map<string, AdvNum>;

    /**
     * 查询某原子的原子量。
     * @param atomName 欲查询的原子名称字符串。
     */
    queryAtom(atomName : string): AdvNum {
        if (this.rmData.has(atomName)) {
            return this.rmData.get(atomName);
        } else {
            throw RMDatabase_Exceptions.AtomNotFoundException;
        }
    }
